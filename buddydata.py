...
def get_gelato_data(splits, data):
    ...
    splits['train'].edge_index = gelato_splits['train']['edge'].t()
    splits['train'].edge_weight = torch.ones(splits['train'].edge_index.size(1), dtype=int)

    splits['valid'].edge_label = torch.hstack([torch.ones(gelato_splits['valid']['edge'].shape[0]), torch.zeros(gelato_splits['valid']['edge_neg'].shape[0])])
    splits['valid'].edge_label_index = torch.vstack([gelato_splits['valid']['edge'], gelato_splits['valid']['edge_neg']]).t()
    splits['valid'].edge_index = gelato_splits['train']['edge'].t()
    splits['valid'].edge_weight = torch.ones(splits['valid'].edge_index.size(1), dtype=int)

    splits['test'].edge_label = torch.hstack([torch.ones(gelato_splits['test']['edge'].shape[0]), torch.zeros(gelato_splits['test']['edge_neg'].shape[0])])
    splits['test'].edge_label_index = torch.vstack([gelato_splits['test']['edge'], gelato_splits['test']['edge_neg']]).t()
    splits['test'].edge_index = torch.vstack([gelato_splits['train']['edge'], gelato_splits['valid']['edge']]).t()
    splits['test'].edge_weight = torch.ones(splits['test'].edge_index.size(1), dtype=int)

    return splits

def get_ncnc_data(args, splits, data):
    
    dataset_file = f"/home/jrm28/fairness/NeuralCommonNeighbor/dataset/splits/{args.dataset_name}.pt" 
    if os.path.isfile(dataset_file):
        ncnc_data, ncnc_splits = torch.load(dataset_file)
    else:
    ...
    splits['train'].edge_label_index = torch.hstack([ncnc_splits['train']['edge'].t(), ncnc_splits['train']['edge_neg'].t()])
    splits['train'].edge_index = ncnc_data.edge_index
    splits['train'].edge_weight = torch.ones(ncnc_data.edge_index.shape[1], dtype=int)
    ...
    splits['valid'].edge_label_index = torch.hstack([ncnc_splits['valid']['edge'].t(), ncnc_splits['valid']['edge_neg'].t()])
    splits['valid'].edge_index = ncnc_data.edge_index
    splits['valid'].edge_weight = torch.ones(ncnc_data.edge_index.shape[1], dtype=int)
    ...
    splits['test'].edge_label_index = torch.hstack([ncnc_splits['test']['edge'].t(), ncnc_splits['test']['edge_neg'].t()])
    splits['test'].edge_index = ncnc_data.edge_index
    splits['test'].edge_weight = torch.ones(ncnc_data.edge_index.shape[1], dtype=int)
...
def get_data(args, dataset_path):
    ...
    elif args.ncnc_splits:
        splits = get_ncnc_data(args, splits, data)
