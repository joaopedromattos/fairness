...
from .utils import scipysp_to_pytorchsp,accuracy,fair_metric, normalize_adjacency
...
class graphair(nn.Module):
    ...
    '''
    def __init__(self, aug_model, f_encoder, sens_model, classifier_model, lr = 1e-4, weight_decay = 1e-5, alpha = 0.1, beta = 1.0, gamma = 10.0, lam = 1.0, dataset = 'POKEC', num_hidden = 64, num_proj_hidden = 64, task=None, split_edge=None, device='cuda:0'):
    ...
    def info_nce_loss_2views(self, h, h_prime, training_pairs=None, training_labels_pairs=None):
        
        features = torch.cat((h, h_prime), dim = 0)
    ...
    def link_pred_loss(self, h, h_prime, training_pairs=None, training_labels_pairs=None):
        features = h_prime
        
        # Method 1: Ignoring the original graph and optimizing the augmented
        # embeddings to link prediction. This is the simlpest way to do it.
        if True:            
            logits = F.cosine_similarity(features[training_pairs[0], :], features[training_pairs[1], :], 1)
            
        # Method 2: This will add a constraint in terms of making the embeddings
        # as informative to link prediction as they were in the original graph.
        # TODO: Implement this method.
        return logits, training_labels_pairs
    ...
    def fit_whole(self, epochs, adj, x,sens,idx_sens,warmup=None, adv_epoches=1):
        ...
        training_pairs = torch.cat((self.split_edge['train']['edge'], self.split_edge['train']['edge_neg'])).t().to(self.device)
        training_labels_pairs = torch.cat((torch.ones(self.split_edge['train']['edge'].shape[0]), torch.zeros(self.split_edge['train']['edge_neg'].shape[0]))).to(self.device)
        
        # contrastive_method = self.link_pred_loss if self.task == 'link_prediction' else self.info_nce_loss_2views
        contrastive_method = self.info_nce_loss_2views
                
        if warmup:
            for _ in range(warmup):
                adj_aug, x_aug, adj_logits = self.aug_model(adj.to(self.device), x, adj_orig = adj_orig.to(self.device))
        ...
        for epoch_counter in range(epochs):
            ### generate fair view
            adj_aug, x_aug, adj_logits = self.aug_model(adj.to(self.device), x.to(self.device), adj_orig = adj_orig.to(self.device))
            ...
            for _ in range(sens_epoches):
                ...
                senloss = self.criterion_sens(s_pred[idx_sens],sens[idx_sens].unsqueeze(2).float().to(self.device))
            ...
            ## update aug model
            logits, labels = contrastive_method(h, h_prime, training_pairs, training_labels_pairs)
    ...
    def fit_batch(self, epochs, adj, x,sens,idx_sens,warmup=None, adv_epoches=1):
        ...
        if not os.path.exists(f"./checkpoint/{self.dataset}_{timestamp}"):
            os.makedirs(f"./checkpoint/{self.dataset}_{timestamp}")
        ...
        training_pairs = torch.cat((self.split_edge['train']['edge'], self.split_edge['train']['edge_neg'])).t()
        training_labels_pairs = torch.cat((torch.ones(self.split_edge['train']['edge'].shape[0]), torch.zeros(self.split_edge['train']['edge_neg'].shape[0])))

        miniBatchLoader = GraphSAINTRandomWalkSampler(Data(x=x, edge_index=edge_index, sens = sens, sens_mask = sens_mask, pairs=training_pairs.to(self.device), pairs_mask=training_labels_pairs.to(self.device), deg = torch.tensor(np.array(adj.sum(1)).flatten())), 
                                                        batch_size = 1000, 
                                                        walk_length = 3, 
                                                        sample_coverage = 500, 
                                                        num_workers = 0,
                                                        save_dir = f"./checkpoint/{self.dataset}_{timestamp}")

        self.miniBatchLoader = miniBatchLoader
        
        if warmup:
            for _ in range(warmup):
                for data in miniBatchLoader:
                    data = data.to(self.device)
                    sub_adj = normalize_adjacency(to_torch_sparse_tensor(data.edge_index, data.edge_norm, size=data.x.shape[0]), data.deg.float()).to(self.device)
                    sub_adj_dense = to_dense_adj(edge_index = data.edge_index, max_num_nodes = data.x.shape[0])[0].float()
                    # import code
                    # code.interact(local={**locals(), **globals()})
                    adj_aug, x_aug, adj_logits = self.aug_model(sub_adj.to(self.device), data.x.to(self.device), adj_orig = sub_adj_dense.to(self.device))  
        ...
        for epoch_counter in range(epochs):
            for data in miniBatchLoader:
                ...
                for _ in range(sens_epoches):
                    ...
                    # Joao's notes: I don't know why they only compute the loss considering the sensitive nodes
                    senloss = torch.nn.BCEWithLogitsLoss(weight=data.node_norm[mask], reduction='sum')(s_pred[mask].squeeze(), data.sens[mask].float())
                ...
                logits, labels = contrastive_method(h, h_prime)
    ...
    def get_fair_graph(self, adj, x):
        ...
        fair_h = self.forward(adj, x.to(self.device))
        ...
        if hasattr(self, 'miniBatchLoader'):
            adj_aug, x_aug, adj_logits = None, None, None
        else:
            adj_aug, x_aug, adj_logits = self.aug_model(adj.to(self.device), x.to(self.device), adj_orig = adj_orig.to(self.device))
        
        
        return adj_aug, x_aug, adj_logits, fair_h
