...
def binarize(A_debiased, adj_ori, threshold_proportion):
    ...
    the_con1 = np.where(
        the_con1 > np.max(the_con1) * threshold_proportion, 1 + the_con1 * 0, the_con1
    )
    the_con1 = np.where(
        the_con1 < np.min(the_con1) * threshold_proportion, -1 + the_con1 * 0, the_con1
    )
...
def metric_wd(feature, adj_norm, flag, weakening_factor, max_hop):
    ...
    print(
        "Sum of all Wasserstein distance value across feature dimensions: "
        + str(sum(emd_distances))
    )
    print(
        "Average of all Wasserstein distance value across feature dimensions: "
        + str(np.mean(np.array(emd_distances)))
    )
...
class EDITS(nn.Module):
    def __init__(
        ...
        self.lr = lr
        self.optimizer_feature_l1 = PGD(
            self.x_debaising.parameters(),
        ...
        self.optimizer_G = torch.optim.RMSprop(
            G_params, lr=self.lr, eps=1e-04, weight_decay=weight_decay
        )
        self.optimizer_A = torch.optim.RMSprop(
            self.fc.parameters(), lr=self.lr, eps=1e-04, weight_decay=weight_decay
        )
    ...
    def forward(self, A, X):
        ...
        agg_con = self.propagation_cat_new_filter(
            X_de.half(), adj_new.half(), layer_threshold=self.layer_threshold
        ).half()
    ...
    def optimize(self, adj, features, idx_train, sens, epoch, lr, alpha=5e-2, beta=0.1):
        ...
        if self.batched:
            return self.optimize_batched(adj, features, idx_train, sens, epoch, lr, alpha, beta)
        else:
            return self.optimize_full(adj, features, idx_train, sens, epoch, lr, alpha, beta)

    def optimize_full(self, adj, features, idx_train, sens, epoch, lr, alpha=5e-2, beta=0.1):
        self.train()
    ...
    def optimize_batched(self, adj, features, idx_train, sens, epoch, lr, alpha=5e-2, beta=0.1):
        self.train()
        self.optimizer_G.zero_grad()
        self.optimizer_feature_l1.zero_grad()
        self.fc.requires_grad_(False)
        ...
        batches = self.create_batches(idx_train)
        
        for batch_idx in batches:
            _, X_debiased, predictor_sens, show, agg_con = self.forward(adj, features)
            positive_eles = torch.masked_select(
                predictor_sens[batch_idx].squeeze(), sens[batch_idx] > 0
            )
            negative_eles = torch.masked_select(
                predictor_sens[batch_idx].squeeze(), sens[batch_idx] <= 0
            )
            ...
            loss_train = alpha * (X_debiased[batch_idx] - features[batch_idx]).norm(2) + beta * adv_loss
        ...
        _, X_debiased, predictor_sens, show, agg_con = self.forward(adj, features)
        adv_loss = self.calculate_adv_loss(predictor_sens, sens, idx_train)
        self.adj_renew.train_adj(X_debiased, adj, adv_loss, epoch, lr)
        ...
        for i in range(8):
            ...
            for batch_idx in batches:
                _, X_debiased, predictor_sens, show, agg_con = self.forward(adj, features)
                loss_train = self.calculate_adv_loss(predictor_sens, sens, batch_idx)
                loss_train.backward()
    ...
    def create_batches(self, idx_train):
        return torch.split(idx_train, self.batch_size)

    def calculate_adv_loss(self, predictor_sens, sens, idx):
        positive_eles = torch.masked_select(
            predictor_sens[idx].squeeze(), sens[idx] > 0
        )
        negative_eles = torch.masked_select(
            predictor_sens[idx].squeeze(), sens[idx] <= 0
        )
        return -(torch.mean(positive_eles) - torch.mean(negative_eles))
    ...
    def fit(
        ...
        for epoch in tqdm(range(epochs)):
            ...
            A_debiased, X_debiased, predictor_sens, show, _ = self.forward(
                adj, features
            )
            positive_eles = torch.masked_select(
                predictor_sens[idx_val].squeeze(), sens[idx_val] > 0
            )
            negative_eles = torch.masked_select(
                predictor_sens[idx_val].squeeze(), sens[idx_val] <= 0
            )
    ...
    def predict(
        ...
        the_con1 = (A_debiased - adj_ori).A
        the_con1 = np.where(
            the_con1 > np.max(the_con1) * threshold_proportion,
            1 + the_con1 * 0,
            the_con1,
        )
        the_con1 = np.where(
            the_con1 < np.min(the_con1) * threshold_proportion,
            -1 + the_con1 * 0,
            the_con1,
        )
        the_con1 = np.where(np.abs(the_con1) == 1, the_con1, the_con1 * 0)
        ...
        def train(epoch, pa, eq, test_f1, val_loss, test_auc):
            ...
            output = model(
                x=X_debiased, edge_index=torch.LongTensor(edge_index.cpu()).cuda()
            )
            ...
            auc_roc_train = roc_auc_score(
                labels.cpu().numpy()[idx_train.cpu().numpy()],
                output.detach().cpu().numpy()[idx_train.cpu().numpy()],
            )
            f1_train = f1_score(
                labels[idx_train.cpu().numpy()].cpu().numpy(),
                preds[idx_train.cpu().numpy()].cpu().numpy(),
            )
            loss_train.backward()
            optimizer.step()
            _, _ = fair_metric(
                preds[idx_train.cpu().numpy()].cpu().numpy(),
                labels[idx_train.cpu().numpy()].cpu().numpy(),
                sens[idx_train.cpu().numpy()].cpu().numpy(),
            )
            ...
            loss_val = F.binary_cross_entropy_with_logits(
                output[idx_val], labels[idx_val].unsqueeze(1).float()
            )
            auc_roc_val = roc_auc_score(
                labels.cpu().numpy()[idx_val.cpu().numpy()],
                output.detach().cpu().numpy()[idx_val.cpu().numpy()],
            )
            f1_val = f1_score(
                labels[idx_val.cpu().numpy()].cpu().numpy(),
                preds[idx_val.cpu().numpy()].cpu().numpy(),
            )
        ...
        def test(test_f1):
            ...
            preds = (
                (output.squeeze() > 0).type_as(labels)[idx_test].detach().cpu().numpy()
            )

            loss_test = F.binary_cross_entropy_with_logits(
                output[idx_test], labels[idx_test].unsqueeze(1).float()
            )
        ...
        for epoch in tqdm(range(epochs)):
            (
                ...
                SP,
                EO,
            ) = train(epoch, pa, eq, test_f1, val_loss, test_auc)
...
class Adj_renew(nn.Module):
    ...
    def fit(self, adj, lr):
        ...
        self.optimizer_adj = optim.SGD(
            estimator.parameters(), momentum=0.9, lr=lr
        )  # 0.005

        self.optimizer_l1 = PGD(
            estimator.parameters(), proxs=[prox_operators.prox_l1], lr=lr, alphas=[5e-4]
        )  # 5e-4
    ...
    def train_adj(self, features, adj, adv_loss, epoch, lr):
        ...
        estimator.estimated_adj.data.copy_(
            (
                estimator.estimated_adj.data
                + estimator.estimated_adj.data.transpose(0, 1)
            )
            / 2
        )
