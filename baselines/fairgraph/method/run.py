...
class run():
    ...
    def run(self,device,dataset,model='Graphair',epochs=10_000,test_epochs=1_000,
        ...
        if model=='Graphair':
            ...
            model = graphair(aug_model=aug_model,f_encoder=f_encoder,sens_model=sens_model,classifier_model=classifier_model, lr=lr,weight_decay=weight_decay,dataset=dataset_name, task=task, split_edge=split_edge).to(device)
        else:
            raise Exception('At this moment, only Graphair is supported!')
        
        if dataset_name == "NBA" or dataset_name=='facebook':
            # call fit_whole
            st_time = time.time()
...
if __name__ == "__main__":
    ...
    run = run()
    adj_aug, x_aug = run.run(device, dataset, model='Graphair', epochs=args.epochs, test_epochs=args.test_epochs, lr=args.lr, weight_decay=args.weight_decay, task=args.task)
    ...
    print("path: ", f"./checkpoint/out/AUGMENTED_{args.dataset}_{args.epochs}_epochs_{timestamp}/splits.pt")
