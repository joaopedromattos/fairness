...
class graphair(nn.Module):
    ...
    '''
    def __init__(self, aug_model, f_encoder, sens_model, classifier_model, lr = 1e-4, weight_decay = 1e-5, alpha = 0.1, beta = 1.0, gamma = 10.0, lam = 1.0, dataset = 'POKEC', num_hidden = 64, num_proj_hidden = 64, task=None, split_edge=None):
    ...
    def fit_whole(self, epochs, adj, x,sens,idx_sens,warmup=None, adv_epoches=1):
        ...
        training_pairs = torch.cat((self.split_edge['train']['edge'], self.split_edge['train']['edge_neg'])).t().cuda()
        training_labels_pairs = torch.cat((torch.ones(self.split_edge['train']['edge'].shape[0]), torch.zeros(self.split_edge['train']['edge_neg'].shape[0]))).cuda()
        
        contrastive_method = self.link_pred_loss if self.task == 'link_prediction' else self.info_nce_loss_2views
                
        if warmup:
            for _ in range(warmup):
                adj_aug, x_aug, adj_logits = self.aug_model(adj.cuda(), x, adj_orig = adj_orig.cuda())
        ...
        for epoch_counter in range(epochs):
            ### generate fair view
            adj_aug, x_aug, adj_logits = self.aug_model(adj.cuda(), x.cuda(), adj_orig = adj_orig.cuda())
            ...
            for _ in range(sens_epoches):
                ...
                senloss = self.criterion_sens(s_pred[idx_sens],sens[idx_sens].unsqueeze(2).float().cuda())
            ...
            if self.task == 'link_prediction':
                contrastive_loss = nn.BCEWithLogitsLoss(reduction='none')(logits, labels).sum()/training_pairs.shape[1]
            else:
    ...
    def fit_batch(self, epochs, adj, x,sens,idx_sens,warmup=None, adv_epoches=1):
        ...
        miniBatchLoader = GraphSAINTRandomWalkSampler(Data(x=x, edge_index=edge_index, sens = sens, sens_mask = sens_mask, pairs=training_pairs.cuda(), pairs_mask=training_labels_pairs.cuda(), deg = torch.tensor(np.array(adj.sum(1)).flatten())), 
                                                        batch_size = 1000, 
                                                        walk_length = 3, 
                                                        sample_coverage = 500, 
                                                        num_workers = 0,
        ...
        for epoch_counter in range(epochs):
            for data in miniBatchLoader:
                ...
                logits, labels = contrastive_method(h, h_prime, data.training_pairs, data.training_labels_pairs)
    ...
    def get_fair_graph(self, adj, x):
        ...
        return adj_aug, x_aug, adj_logits
