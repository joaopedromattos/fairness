...
class CNLinkPredictor(nn.Module):
    ...
    def multidomainforward(self,
        ...
        xj = x[tar_ei[1]]
        x = x + self.xlin(x)
        cn = adjoverlap(adj.cpu(), adj.cpu(), tar_ei, filled1, cnsampledeg=self.cndeg)
       
        xcns = [spmm_add(cn.cpu(), x.cpu())]
        xij = self.xijlin(xi * xj)
        
        xs = torch.cat(
            [self.lin(self.xcnlin(xcn.to(xij.device)) * self.beta + xij) for xcn in xcns],
            dim=-1)
