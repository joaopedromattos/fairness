...
def gradient_penalty(embs, A, batch_size, seed, fair_model):
    def _xor(x, y):
        x_or_y = torch.min(x + y, torch.tensor(1.0))
        not_x_and_y = 1.0 - x * y
        return torch.squeeze(x_or_y * not_x_and_y)

    idx = torch.randperm(batch_size, generator=torch.Generator().manual_seed(seed))
    
    embsS = embs[idx, :]  # embs shuffled -> Joao's notes - We are not shuffling edges right now
    As = A[idx]  # a shuffled
    # Ys = Y[idx]  # y shuffled

    u = torch.rand((batch_size, 1), device=embs.device)
    embsmix = u * embs + (1 - u) * embsS  # a minibatch of randomly convexly mixed embs and embsS
    # Ymix = u * Y + (1 - u) * Ys  # a minibatch of randomly convexly mixed Y and Ys

    embsmix.requires_grad_(True)
    # Ymix.requires_grad_(True)

    Ahat_mix = fair_model(embsmix)

    A_xor_As = _xor(A, As)  # one iff original and shuffled data belong to diff groups
    scale_factor = batch_size / torch.sum(A_xor_As)

    gradients = torch.autograd.grad(
        outputs=Ahat_mix,
        inputs=embsmix,
        grad_outputs=torch.ones_like(Ahat_mix),
        create_graph=True,
        retain_graph=True,
    )[0]

    gradients_norm = torch.norm(gradients, dim=1)
    gradient_penalty = F.mse_loss(
        gradients_norm,
        torch.ones_like(gradients_norm),
        reduction='none'
    )
    
    return scale_factor * torch.mean(gradient_penalty * A_xor_As)
...
def train(model,
    ...
    for perm in PermIterator(adjmask.device, adjmask.shape[0], min(adjmask.shape[0], batch_size)):
        ...
        if nifty_loss:
            flipped_x = data.x.clone() # do not remove the clone
        ...
        if not no_intervention:
            ...
            if nifty_loss:
                if link_level:
                    ...
                    z1 = torch.cat([h[pos_train_edge[:, perm][0]], h[pos_train_edge[:, perm][1]]], dim=-1)
                    # z1_neg = torch.cat([h[negedge[:, perm][0]], h[negedge[:, perm][1]]], dim=-1)
                    # z1 = torch.cat([z1_pos, z1_neg])
                    
                    z2 = torch.cat([h_flipped[pos_train_edge[:, perm][0]], h_flipped[pos_train_edge[:, perm][1]]], dim=-1)
                    # z2_neg = torch.cat([h_flipped[negedge[:, perm][0]], h_flipped[negedge[:, perm][1]]], dim=-1)
                    # z2 = torch.cat([z2_pos, z2_neg])
                ...
                # loss = (1 - reg_lambda) * (neg_losss + pos_losss) - (reg_lambda * fairness_loss)
                loss = (1 - reg_lambda) * (neg_losss + pos_losss) + (reg_lambda * fairness_loss)
                ...
                if link_level:
                    ...
                    if args.laftr_dp:
                        ...
                        # For equality of opportunity we will only consider the positive pairs
                        # as seen in the original LAFTR paper.
                        
                        h_f_a = fair_pred.softmax(dim=-1)

                        pos_pairs_fair_outputs = h_f_a[:pos_embs.shape[0], :]

                        pos_pairs_fair_labels = labels[:pos_embs.shape[0]].argmax(1)

                        A1_Y1 = pos_pairs_fair_labels == 1

                        A0_Y1 = pos_pairs_fair_labels != 1

                        pos_pairs_fair_outputs_A1_Y1 = pos_pairs_fair_outputs[A1_Y1, 1]

                        pos_pairs_fair_outputs_A0_Y1 = pos_pairs_fair_outputs[A0_Y1, 0]
                        
                        gp = gradient_penalty(embs[:pos_embs.shape[0], :], pos_pairs_fair_labels, batch_size, 0, fair_model)

                        fairness_loss = ((1 - pos_pairs_fair_outputs_A1_Y1).abs().sum() * (1/A1_Y1.sum())) + ((1 - pos_pairs_fair_outputs_A0_Y1).abs().sum() * (1/A0_Y1.sum())) + args.gp_coef * gp
                        
                        # import code
                        # code.interact(local={**locals(), **globals()})
                        
                        if is_model_trainable(fair_model):
                            print("Fair model trainable")
                            loss = fairness_loss # We want to maximize the loss
                            loss.backward()
                            ...
                            fair_optimizer.step()
                        elif is_model_trainable(model):
                            print("Model trainable")
                            loss = 1 * (neg_losss + pos_losss) - (reg_lambda * fairness_loss)
                            loss.backward()
                            optimizer.step()
                        ...
                        fairness_loss = F.binary_cross_entropy(fair_pred.softmax(-1), labels)
                    ...
                    if args.laftr_eo:
                        ...
                        link_repr = torch.cat([h[pos_train_edge[:, perm][0]], h[pos_train_edge[:, perm][1]]], dim=-1)
                        
                        fair_pred = fair_model(link_repr).softmax(-1)

                        pos_pairs_fair_outputs = fair_pred

                        pos_pairs_fair_labels = (data.y[pos_train_edge[:, perm]].sum(0).long() == 1).float()
                        
                        labels = torch.nn.functional.one_hot(pos_pairs_fair_labels.long(), num_classes=len(pos_pairs_fair_labels.unique())).float()
                        ...
                        gp = gradient_penalty(link_repr, pos_pairs_fair_labels, batch_size, 0, fair_model)
                ...
                batch_acc = binary_accuracy(fair_pred.softmax(-1).cpu(), labels.cpu()).item()
...
def test(model, fair_model, predictor, data, split_edge, evaluator, batch_size, link_level,
    ...
    if not link_level:
        ...
        if args.laftr_eo:
            accs = []
            for pos_edge, neg_edge, set_name in [(pos_train_edge, neg_train_edge, 'train'), (pos_valid_edge, neg_valid_edge, 'valid'), (pos_test_edge, neg_test_edge, 'test')]:
                link_repr_pos = torch.cat([h[pos_edge[:, 0]], h[pos_edge[:, 1]]], dim=-1)
                link_repr_neg = torch.cat([h[neg_edge[:, 0]], h[neg_edge[:, 1]]], dim=-1)
                link_repr = torch.vstack([link_repr_pos, link_repr_neg])
                
                
                pos_pairs_fair_labels = (data.y[pos_edge].sum(1).long() == 1).float()
                neg_pairs_fair_labels = (data.y[neg_edge].sum(1).long() == 1).float()
                fair_labels = torch.cat([pos_pairs_fair_labels, neg_pairs_fair_labels])
                
                # one hot encoding the labels
                fair_labels = torch.nn.functional.one_hot(fair_labels.long(), num_classes=len(fair_labels.unique())).float()

                fair_pred = fair_model(link_repr)
                
                fairness_acc = binary_accuracy(fair_pred.softmax(-1).cpu(), fair_labels.cpu())
                accs.append(fairness_acc)
                
            fairness_train, fairness_valid, fairness_test = accs
...
def main():
    ...
    dataset_paths = {
        ...
        'facebook_nifty': "/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/dataset/splits/facebook_nifty.pt",
        'facebook_edits': "/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/dataset/splits/facebook_edits.pt",
        'facebook_gae':'/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/gemb/NCN_facebook_facebook_puregcn_cn1_256.pt',
        'nba': "/home/jrm28/fairness/data/graphs/nba.pt",
        'german': "/home/jrm28/fairness/data/graphs/german.pt",
    ...
    dataset_file = f"/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/dataset/splits/{args.dataset}{'_node_split' if args.node_split else ''}.pt" 
    ...
    if gae_embedding_experiment:
        dataset_paths_gae = {
            'facebook': "/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/gemb/NCN_facebook_facebook_puregcn_cn1_256.pt",
        }[args.dataset]
        data.x = torch.load(dataset_paths_gae)
        
        
    if args.fairsample:
        if os.path.isfile(f'/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/dataset/splits/fairsample_{args.dataset}.pt'):
            logger.info("Loading fair samples...")
            data, split_edge = torch.load(f'/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/dataset/splits/fairsample_{args.dataset}.pt')
        else:
            logger.info("Sampling fair samples...")
            data, split_edge = fairsample(data, split_edge)
            torch.save((data, split_edge), f'/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/dataset/splits/fairsample_{args.dataset}.pt')
    ...
    if args.nifty:
        ...
        input_size = args.hiddim * 2 if args.concat else args.hiddim
        projection_layer1 = torch.nn.Linear(input_size, input_size).to(device=device)
        projection_layer2 = torch.nn.Linear(input_size, input_size).to(device=device)
        projection_layers = (projection_layer1, projection_layer2)
        
    # create random features
    if args.random_features:
        data.x = torch.randn(data.x.size(0), 128).to(device)
    ...
    if args.predictor in ["cn1", "cn1_mod", "incn1cn1", "scn1", "catscn1", "sincn1cn1", 'decoder']:
    ...
    for run in range(0, args.runs):
        ...
        run_name += 'CONCAT - ' if args.concat else ''
        ...
        if args.wandb_sweep:
            ...
            args.gp_coef = wandb.config.gp_coef
        ...
        fair_model_input = args.hiddim * 2 if args.laftr_eo and not args.link_level else args.hiddim
        fair_model = (FairLearner_GNN if args.fair_learner_gnn else FairLearner)(fair_model_input, fair_model_input * 2, hidden_layers=4, out_channels=2).to(device)
        ...
        alternate_training = 1 # Controls whether we are training the fair model or the predictor model.
        
        # Giving a headstart to the generator by disabling the fair model.
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            if epoch % 50 == 0 and args.alternate_training:
                alternate_training = 1 - alternate_training
                print("Alternate training", alternate_training)
            ...
            if True:
                ...
                if True:
                    if bestscore is None or bestscore[f"rep{0}_ValHits@{100}"] < results[f"rep{0}_ValHits@{100}"]:
                        ...
                        torch.save(saved_output, f"/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/saved_output_new/{args.dataset}_{args.model}_{args.predictor}_{args.hiddim}_{run}_{epoch}{timestamp}.pt")
                        ...
                        if args.save_gemb:
                            torch.save(h, f"/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/gemb/{run_name.replace(' ', '_')}_{args.dataset}_{args.model}_{args.predictor}_{args.hiddim}.pt")
                        if args.savex:
                            torch.save(model.xemb[0].weight.detach(), f"/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/gemb/{run_name.replace(' ', '_')}_{args.dataset}_{args.model}_{args.predictor}_{args.hiddim}_{run}.pt")
                        if args.savemod:
                            torch.save(model.state_dict(), f"/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/gmodel/{run_name.replace(' ', '_')}_{args.dataset}_{args.model}_{args.predictor}_{args.hiddim}_{run}.pt")
                            torch.save(predictor.state_dict(), f"/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/gmodel/{run_name.replace(' ', '_')}_{args.dataset}_{args.model}_{args.predictor}_{args.hiddim}_{run}.pre.pt")
                    
                    print(f'Run: {run + 1:02d}, '
                            f'Epoch: {epoch:02d}, '
                            f'Loss: {loss:.4f}, '
                            f'Fair Loss: {fair_loss:.4f}, '
                            f'Best epoch ({best_epoch}) - Train: {100 * train_hits:.2f}%, '
        ...
        if args.node_classification:
            ...
            if Path(f"/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/gemb/{run_name.replace(' ', '_')}_{args.dataset}_{args.model}_{args.predictor}_{args.hiddim}.pt").exists():
                logger.info("Loading embeddings from disk...")
                h = torch.load(f"/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/gemb/{run_name.replace(' ', '_')}_{args.dataset}_{args.model}_{args.predictor}_{args.hiddim}.pt")
            
            if Path(f"/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/gmodel/{run_name.replace(' ', '_')}_{args.dataset}_{args.model}_{args.predictor}_{args.hiddim}_{run}.pt").exists():
                logger.info("Loading model from disk...")
                model.load_state_dict(torch.load(f"/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/gmodel/{run_name.replace(' ', '_')}_{args.dataset}_{args.model}_{args.predictor}_{args.hiddim}_{run}.pt"))
                
            if Path(f"/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/gmodel/{run_name.replace(' ', '_')}_{args.dataset}_{args.model}_{args.predictor}_{args.hiddim}_{run}.pre.pt").exists():
                logger.info("Loading predictor from disk...")
                predictor.load_state_dict(torch.load(f"/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/gmodel/{run_name.replace(' ', '_')}_{args.dataset}_{args.model}_{args.predictor}_{args.hiddim}_{run}.pre.pt"))
            ...
            # Save results
            torch.save((y_pred, labels[data.test_mask, :]), f"/home/jrm28/fairness/in_processing_methods/NeuralCommonNeighbor/saved_output/{args.dataset}_{args.model}_{args.predictor}_{args.hiddim}_{run}_node_classification.pt")
