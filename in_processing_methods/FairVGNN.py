...
def evaluate_ged3(x, classifier, discriminator, generator, encoder, data, valid_edge_index, test_edge_index, args):
    classifier.eval()
    generator.eval()
    encoder.eval()
    ...
    with torch.no_grad():
        if(args.f_mask == 'yes'):
            ...
            for k in range(args.K):
                ...
                output = classifier(h, data.edge_index, test_edge_index)
                # output2 = discriminator(h)
...
class Classifier(nn.Module):
    def __init__(self, args):
        classifier_name, ft_in, nb_classes = args.classifier_name, args.hidden, args.num_classes
...
class FairVGNN():
    ...
    def run(self, data, args):
        ...
        for count in range(args.runs):
            ...
            for epoch in tqdm(range(0, args.epochs)):
                ...
                for epoch_c in range(0, args.c_epochs):
                    ...
                    if (args.f_mask == 'yes'):
                        ...
                        for k in range(args.K):
                            x = data.x * masks[k].detach()
                            h = encoder(x, data.edge_index, data.adj_norm_sp)
                            output = classifier(h, data.edge_index, self.train_edge_index)

                            loss_c += F.binary_cross_entropy_with_logits(
                                output, self.train_edge_labels.to(args.device))
                ...
                test_output = evaluate_ged3(
                    data.x, classifier, discriminator, generator, encoder, data, self.valid_edge_index, self.test_edge_index, args)
    ...
    def fit(self,adj, feats, labels, idx_train, idx_val, idx_test, sens, sens_idx, edge_splits, device='cuda:0', runs=1, epochs=200, d_epochs=5, g_epochs=5,
            c_epochs=5, g_lr=0.001, g_wd=0, d_lr=0.001, d_wd=0, c_lr=0.001, c_wd=0, e_lr=0.001, e_wd=0, early_stopping=0,
            prop='spmm', dropout=0.5, hidden=16, seed=1, encoder='GCN', classifier_name='gae', K=10, top_k=10, clip_e=1, f_mask='yes',weight_clip='yes',ratio=1, alpha=1):
        ...
        self.args=args
        print('running')
        test_output = self.run(data, args)
        return test_output
